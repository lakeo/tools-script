...
function initListener() {
  ...
  document.getElementById('cancelAllUserBtn').onclick = function(){
    ...
    document.getElementById('openAutoCancelNotVipModelBtn').onclick = function(){
        checkTime(function(){
            chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
                chrome.tabs.sendMessage(tabs[0].id, {cmd: "openAutoCancelNotVipModelBtn",alert:false, msg:''}, function(response) {
                    console.log(response);
                });
            });
        })
