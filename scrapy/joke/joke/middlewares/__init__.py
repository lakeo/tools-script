...
class DuplicateDownloaderMidderware(UserAgentMiddleware):
    ...
    def process_request(self, request, spider):
        if request.url in self.seen:
            spider.logger.info('duplicate url %s' % request.url)
            raise IgnoreRequest()
        else:
...
class MyCustomDownloaderMiddleware(UserAgentMiddleware):
    ...
    def process_request(self, request, spider):
        ...
        request.headers.setdefault('User-Agent', 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/47.0.2526.111 Safari/537.36')
        request.headers.setdefault('Referer','http://www.qiushibaike.com/')
        request.headers.setdefault('Accept-Encoding','gzip, deflate, sdch')
        request.headers.setdefault('Accept-Language','zh-CN,zh;q=0.8,en;q=0.6,pt;q=0.4,zh-TW;q=0.2')
